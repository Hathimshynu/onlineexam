...
class User extends CI_Controller
{
    ...
    /**
     * The `saveexam` function saves the user's answers to an exam, calculates the number of correct and
     * wrong answers, and returns the results in JSON format.
     */
    public function saveexam()
    {
        if ($this->session->userdata('reguser')) {
            $username = $this->session->userdata('reguser');

            // Define the correct answers
            $correct_answers = ['paris', 'camel', '7 days', '24 hours', '26 letters', '7 colours', '365 days', '60 minutes', 'tiger', 'mango'];
            ...
            log_message('error', 'requestdata' . $requestData);
            ...
            for ($i = 1; $i <= 10; $i++) {
                $selectedAnswer = $requestData["question{$i}"];
                $question = "Question {$i}";
                log_message('error', 'selected answer' . $selectedAnswer);
                // Check if the selected answer is in the array of correct answers
                if (in_array($selectedAnswer, $correct_answers)) {
                    $isCorrect = true;
                    $numCorrect++;
                } else {
                    $isCorrect = false;
                ...
                // Save the result to the database using a model method
                $resultId = $this->user->saveResult($username, $question, $selectedAnswer, $correct_answers[$i - 1], $isCorrect);
